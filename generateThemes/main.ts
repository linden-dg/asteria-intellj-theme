...
import {colormath, Handlebars, path} from "./deps.ts";
import {variants} from "./themes.ts";
...
Deno.readTextFile(uiPath).then((data) => {
  Object.entries(variants).forEach(([key, value]) => {
    ...
    Deno.writeTextFileSync(path.join(themePath, `${key}.json`), JSON.stringify(value, null, 2));
